...
equip_id,equip_desc,equip_type_id,physical_location,equip_state_id,equip_state_in_datetime,equip_state_out_datetime,event_code_id, event_hist_emp_no,semi_state_id --equip_note.note_text
   ...
   WHERE  semi_state_id = 'SCHEDULED_DOWNTIME' AND equip_state_in_datetime>'2022-08-10 07:00:00' AND equip_state_id like 'IN_PM%' OR equip_state_id like 'PM_ES%'
...
df1 = pd.read_sql(cmd,cnxn)
df1 = df1.head(1000)

df1["equip_state_in_datetime"] = pd.to_datetime(df1["equip_state_in_datetime"]).dt.strftime('%Y/%m/%d %H:%M:%S')
df1["equip_state_out_datetime"] = pd.to_datetime(df1["equip_state_out_datetime"]).dt.strftime('%Y/%m/%d %H:%M:%S')


# df2 = df2.drop(columns='1')
# df2 = df2['Event code (ID)'].fillna(0, inplace=True)
df1.to_excel("C:/Users/itanb/test_site2/Scripts/TestIvan2/TestApp2/PMstorage/PM_data/pmrecord.xlsx")
